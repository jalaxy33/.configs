...
选择 `CLI + Skills` > 选择 `OpenCode`。

之后会在 `~/.agents/skills/` 目录下安装一个名为 `find-docs` 的 skill，然后将这个 skill 软链接到 `~/.claude/skills/` 目录下。

> 提示：软链接这一步可以在安装好 skill 之后，在 cc-switch 的 skills 管理中「导入已有」skills，并为 claude 激活。
